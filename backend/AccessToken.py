...
class AccessToken:
    ...
    def build(self):
        ...
        val = (self.appID + self.channelName + self.uidStr).encode() + m
        signature = hmac.new(self.appCertificate.encode(), val, sha256).digest()
        crc_channel_name = crc32(self.channelName.encode()) & 0xffffffff
        crc_uid = crc32(self.uidStr.encode()) & 0xffffffff
        ...
        version = getVersion()
        return (version + self.appID + base64.b64encode(content).decode())
